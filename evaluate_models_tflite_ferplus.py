"""
Valuta i modelli TFLite sul test set di FERPlus e confronta l'accuracy con i modelli .keras
...
TFLITE_DIR = os.path.join(BASE_DIR, "tflite_models_ferplus") # cartella nuova ferplus
DATA_DIR = os.path.join(BASE_DIR, "FERPlus") # dataset FERPlus
RESULTS_DIR = os.path.join(BASE_DIR, "tflite_evaluation_ferplus") # output separato
os.makedirs(RESULTS_DIR, exist_ok=True)


# FERPlus ha 8 classi (aggiunta 'contempt' rispetto a FER2013)
CLASSES = ['angry', 'contempt', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise']
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")


# Accuracy originale dei .keras addestrati su FERPlus
KERAS_ACCURACY = {
    "EfficientNetV2B2": 81.81,
    "EfficientNetB0":   81.89,
    "MobileNetV3Small": 76.13,
}
...
def load_interpreter(tflite_path, name):
    ...
    # Verifica canali: FERPlus è grayscale → atteso 1 canale
    actual_channels = input_details[0]['shape'][-1]
    if actual_channels != 1:
        print(f"  ATTENZIONE - Input ha {actual_channels} canali invece di 1!")
        print(f"  Impostare USE_RGB_INPUT = True")
    else:
        print(f"  Input shape corretto: {actual_channels} canale grayscale")

    # Verifica output: FERPlus ha 8 classi
    actual_output = output_details[0]['shape'][-1]
    if actual_output != len(CLASSES):
        print(f"  ATTENZIONE - Output ha {actual_output} neuroni, attesi {len(CLASSES)}!")
    else:
        print(f"  Output corretto: {actual_output} classi FERPlus")

    return interpreter, input_details, output_details


# Preparazione del test set FERPlus
def prepare_test_set(img_size, batch_size=32):
    datagen = ImageDataGenerator()  # nessun rescale
    test_set = datagen.flow_from_directory(
        os.path.join(DATA_DIR, "test"),
        target_size=(img_size, img_size),
        color_mode="grayscale",     # 1 canale, come nel training FERPlus
        batch_size=batch_size,
        class_mode="categorical",
        classes=CLASSES,            # 8 classi FERPlus
...
def run_inference(interpreter, input_details, output_details, test_set):
    ...
    while processed < test_set.n:
        ...
        for img in batch_images:
            ...
            img_input = img_input.astype(in_dtype)
...
def save_outputs(y_pred, y_true, name, accuracy, delta, report, lat_mean, lat_p95):
    ...
    fig, ax = plt.subplots(figsize=(10, 8))   # leggermente più grande per 8 classi
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=CLASSES)
    disp.plot(ax=ax, cmap="Blues", colorbar=False, xticks_rotation=45)
    delta_str = f"{delta:+.2f} pp vs Keras" if delta else ""
    ax.set_title(
        f"Confusion Matrix TFLite FERPlus - {name}\nAccuracy: {accuracy:.2f}% ({delta_str})",
        fontsize=12
    )
    plt.tight_layout()
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"Dataset: FERPlus (8 classi)\n")        # aggiunto FERPlus
...
if __name__ == "__main__":
    ...
    for cfg in MODELS:
        ...
        name = cfg["name"]
        print(f"\n\n{'#'*60}")
        print(f"  VALUTAZIONE TFLITE FERPlus: {name}")   # BUG FIXATO: era [name] invece di {name}
        print(f"{'#'*60}")

        interpreter, input_details, output_details = load_interpreter(cfg["tflite_path"], name)
        print(f"[2/5] Preparazione test set FERPlus (img_size = {cfg['img_size']})...")
        test_set = prepare_test_set(cfg["img_size"])

        y_pred, y_true, lat_mean, lat_p95 = run_inference(
            interpreter, input_details, output_details, test_set
        )
    ...
    # Riepilogo finale
    print(f"\n\n{'='*65}")
    print(f"  CONFRONTO FINALE KERAS vs TFLITE — FERPlus")
    print(f"{'='*65}")
    print(f"  {'Modello':<22} {'Keras':>8} {'TFLite':>8} {'Delta':>8} {'Lat':>8}")
    print(f"  {'-'*60}")
