...
model.compile(
    ...
    loss = CategoricalCrossentropy(label_smoothing=0.10), #l'uso del label smoothing aiuta a prevenire l'overfitting, specialmente in un dataset relativamente piccolo e sbilanciato come il FER
    metrics = ['accuracy']
)
...
history_head = model.fit(
    ...
    epochs = 5, #Aggiunta di epoch dovuta all'aumento dei parametri
...
reduce_lr_ft = ReduceLROnPlateau(
    ...
    patience=4, #aumento della pazienza per permettere al modello di stabilizzarsi dopo il cambio di learning rate
    verbose=1,
    min_delta=0.001    
)
...
history_ft = model.fit(
    train_set,
    validation_data=validation_set,
    epochs=40, #40 epoch per una migliore ottimizzazione con LR decay, con early stopping per prevenire overfitting
...
json_path = os.path.join(MODEL_DIR, f"history_mobilenetv3small_{timestamp}.json")
...
axes[0].set_title('Accuracy - MobileNetV3Small')
...
axes[1].set_title('Loss - MobileNetV3Small')
