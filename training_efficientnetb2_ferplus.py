"""
Addestra il modello per il riconoscimento delle emozioni facciali EfficientNetB2V2, pre-addestrato su ImageNet, sul dataset FERPlus
il dataset FERPlus comprende 8 classi: le 7 di Ekman (angry, disgust, fear, happy, neutral, sad e surprise) + contempt
...
#Output layer per le 8 classi FERPlus (aggiunge contempt)
...
MODEL_DIR = os.path.join(BASE_DIR, "models", "efficientnetv2b2_ferplus") #al fine del confronto tra modelli, è importante salvare i modelli in una cartella dedicata all'interno del progetto, in modo da poterli confrontare facilmente e mantenere il progetto organizzato
...
checkpoint = ModelCheckpoint(
    os.path.join(MODEL_DIR, "best_model_v2b2_ferplus.keras"), #salvataggio del modello nella fase 1
...
checkpoint_ft = ModelCheckpoint(
    os.path.join(MODEL_DIR, "best_model_v2b2_ferplus_ft.keras"),  # file separato
    monitor='val_accuracy', verbose=1, save_best_only=True, mode='max'
)
early_stopping_ft = EarlyStopping(
    monitor='val_accuracy', patience=5, verbose=1, restore_best_weights=True, mode='max'
)
...
json_path = os.path.join(MODEL_DIR, f"history_v2b2_ferplus_{timestamp}.json")
...
axes[0].set_title('Accuracy - EfficientNetV2B2 FERPlus')
...
axes[1].set_title('Loss - EfficientNetV2B2 FERPlus')
...
plt.tight_layout()
plot_path = os.path.join(MODEL_DIR, f"training_plot_v2b2_ferplus_{timestamp}.png")
